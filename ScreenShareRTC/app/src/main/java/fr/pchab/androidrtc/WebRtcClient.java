...
public class WebRtcClient {
    ...
    Context mContext;
    // 制定adb命令的lei
    private ProcessBuilder mBuilder =new ProcessBuilder();
...
//                    JSONObject data = new JSONObject(info);
        ...
        // input
        public Emitter.Listener onInput = new Emitter.Listener() {
            @Override
            public void call(Object... args) {
                JSONObject data = (JSONObject) args[0];
                String from = data.optString("from");
                String type = data.optString("type");
                String inputData = data.optString("input");

                mListener.onStatusChanged("inputData:"+inputData);
                Log.d(TAG, "input socket received " + type + " from " + from+" inputData"+inputData);
                String [] subInput = inputData.split(" ");
...
//                    // 获取输出流
                    ...
                    DataOutputStream dataOutputStream = new DataOutputStream(
                            outputStream);
                    dataOutputStream.writeBytes("input tap " + subInput[0]+ " " + subInput[1]);
...
//                String[] order = {"input", "tap", "" + subInput[0], "" + subInput[1]};
//                String[] order = {"input", "tap", "" + subInput[0], "" + subInput[1]};
//                String[] order = {"input", "tap", "" + Integer.parseInt(subInput[0]), "" + Integer.parseInt(subInput[1])};

//
//                try {
//                    mBuilder.command("input tap " + subInput[0]+ " " + subInput[1]).start();
...
//            mListener.onAddRemoteStream(mediaStream, endPoint + 1);
    ...
    public WebRtcClient(Context context, RtcListener listener, VideoCapturer capturer, PeerConnectionClient.PeerConnectionParameters params) {
        ...
        mSocket.on("input", messageHandler.onInput);
        ...
        iceServers.add(new PeerConnection.IceServer("stun:172.16.211.23:3478"));
        iceServers.add(new PeerConnection.IceServer("stun:172.16.211.24:3478"));
